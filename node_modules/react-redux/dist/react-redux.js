(function (global, factory) {
	...
	*/
	 ...
	 * @param {string} location e.g. "prop", "contexts", "child contexts"
	...
	var factoryWithTypeCheckers = function(isValidElement, throwOnDirectAccess) {
	   ...
	   *
	   * Be sure to invoke the function with the iterable as contexts:
	   *
	      ...
	      for (var key in allKeys) {
	        ...
	        if (!checker) {
	          return new PropTypeError(
	            'Invalid ' + location + ' `' + propFullName + '` keycode `' + key + '` supplied to `' + componentName + '`.' +
	...
	*/
	...
	_ref) {
	  ...
	  var customStoreWarningMessage = 'To use a custom Redux store for specific components, create a custom React contexts with ' + "React.createContext(), and pass the contexts object to React Redux's Provider and specific components" + ' like: <Provider contexts={MyContext}><ConnectedComponent contexts={MyContext} /></Provider>. ' + 'You may also pass a {contexts : MyContext} option to connect';
	  ...
	  return function wrapWithConnect(WrappedComponent) {
	    ...
	    function ConnectFunction(props) {
	      var _useMemo = React.useMemo(function () {
	        ...
	        // and values needed to control behavior (forwarded refs, alternate contexts instances).
	      ...
	      var ContextToUse = React.useMemo(function () {
	        // Users may optionally pass in a custom contexts instance to use instead of our ReactReduxContext.
	        // Memoize the check that determines which contexts instance we should use.
	      ...
	      }, [propsContext, Context]); // Retrieve the store and ancestor subscription via contexts, if available

	      var contextValue = React.useContext(ContextToUse); // The store _must_ exist as either a prop or in contexts

	      var didStoreComeFromProps = Boolean(props.store);
	      var didStoreComeFromContext = Boolean(contextValue) && Boolean(contextValue.store);
	      invariant_1(didStoreComeFromProps || didStoreComeFromContext, "Could not find \"store\" in the contexts of " + ("\"" + displayName + "\". Either wrap the root component in a <Provider>, ") + "or pass a custom React contexts provider to <Provider> and the corresponding " + ("React contexts consumer to " + displayName + " in connect options."));
	      ...
	      var _useMemo2 = React.useMemo(function () {
	        if (!shouldHandleStateChanges) return NO_SUBSCRIPTION_ARRAY; // This Subscription's source should match where store came from: props vs. contexts. A component
	        // connected to the store via props shouldn't use subscription from contexts, or vice versa.
	        ...
	        return [subscription, notifyNestedSubs];
	          ...
	          notifyNestedSubs = _useMemo2[1]; // Determine what {store, subscription} value should be put into nested contexts, if necessary,
	      // and memoize that value to avoid unnecessary contexts updates.
	      ...
	      var overriddenContextValue = React.useMemo(function () {
	        if (didStoreComeFromProps) {
	          ...
	          // the existing contexts value is from the nearest connected ancestor.
	          return contextValue;
	        } // Otherwise, put this component's subscription instance into contexts, so that
	      ...
	      var renderedChild = React.useMemo(function () {
	        if (shouldHandleStateChanges) {
	          ...
	          // Context instance, and putting a different value into the contexts.
	...
	function useReduxContext() {
	  ...
	  invariant_1(contextValue, 'could not find react-redux contexts value; please ensure the component is wrapped in a <Provider>');
	  return contextValue;
	}

	/**
	 * Hook factory, which creates a `useStore` hook bound to a given contexts.
	 *
	 * @param {Function} [context=ReactReduxContext] Context passed to your `<Provider>`.
	 * @returns {Function} A `useStore` hook bound to the specified contexts.
	 */
	...
	/**
	 * Hook factory, which creates a `useDispatch` hook bound to a given contexts.
	 *
	 * @param {Function} [context=ReactReduxContext] Context passed to your `<Provider>`.
	 * @returns {Function} A `useDispatch` hook bound to the specified contexts.
	 */
	...
	/**
	 * Hook factory, which creates a `useSelector` hook bound to a given contexts.
	 *
	 * @param {Function} [context=ReactReduxContext] Context passed to your `<Provider>`.
	 * @returns {Function} A `useSelector` hook bound to the specified contexts.
	 */
